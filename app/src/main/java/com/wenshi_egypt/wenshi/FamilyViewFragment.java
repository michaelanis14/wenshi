...
public class FamilyViewFragment extends Fragment implements View.OnClickListener {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        demo = getView().findViewById(R.id.familyViewDemo);
        final ArrayList<String> results = new ArrayList<>();
        rootRef = FirebaseDatabase.getInstance().getReference();

        rootRef.child("FamilyView").orderByKey().addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                HashMap<String, String> value = (HashMap<String, String>) dataSnapshot.getValue();
                step1 = value.get("demoFamilyView");
                demo.setText(step1);
            }
